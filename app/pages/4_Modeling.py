...
# ============================================================
# PARTIAL DEPENDENCE PLOTS (Seatbelt & Airbag Conditions)
# ============================================================

st.header("Partial Dependence Plots: Seatbelt and Airbag Conditions")

st.write(
    "These partial dependence plots isolate how the model's predicted severity "
    "changes with age under different protective‑equipment conditions. "
    "They help visualize how seatbelt use and airbag deployment influence "
    "the model's predictions for both non‑severe and severe injury outcomes."
)

from sklearn.inspection import PartialDependenceDisplay

# -----------------------------
# PDP: Seatbelt — Non-Severe (target = 3)
# -----------------------------
st.subheader("Non-Severe Prediction • Without Seatbelt")

fig, ax = plt.subplots(figsize=(6, 4))
PartialDependenceDisplay.from_estimator(
    clf,
    X[X['Seatbelt Grouped_No Restraints Used'] == 1],
    features=['Age'],
    target=3,
    ax=ax
)
...
# -----------------------------
# PDP: Seatbelt — Severe (target = 5) WITH seatbelt
# -----------------------------
st.subheader("Severe Prediction • With Seatbelt")

fig, ax = plt.subplots(figsize=(6, 4))
PartialDependenceDisplay.from_estimator(
    clf,
    X[X['Seatbelt Grouped_No Restraints Used'] == 0],
    features=['Age'],
    target=5,
    ax=ax
)
...
# -----------------------------
# PDP: Seatbelt — Severe (target = 5) WITHOUT seatbelt
# -----------------------------
st.subheader("Severe Prediction • Without Seatbelt")
...
# -----------------------------
# PDP: Airbag — Non-Severe (target = 3) WITH deployed airbag
# -----------------------------
st.subheader("Non-Severe Prediction • Deployed Airbag (Front Only)")

mask_deployed_front = (
    (X['Airbag Grouped_Airbag Equipped - Not Activated'] == 0) &
    (X['Airbag Grouped_Not Airbag Equipped'] == 0)
)
X_deployed_front = X[mask_deployed_front]
...
# -----------------------------
# PDP: Airbag — Non-Severe (target = 3) WITHOUT deployed airbag
# -----------------------------
st.subheader("Non-Severe Prediction • Without Deployed Airbag (Front)")

mask_not_deployed_front = (
    (X['Airbag Grouped_Airbag Equipped - Not Activated'] == 1) |
    (X['Airbag Grouped_Not Airbag Equipped'] == 1)
)
X_not_deployed_front = X[mask_not_deployed_front]

fig, ax = plt.subplots(figsize=(6, 4))
PartialDependenceDisplay.from_estimator(
    clf,
    X_not_deployed_front,
    features=['Age'],
    target=3,
    ax=ax
)
...
# -----------------------------
# PDP: Airbag — Severe (target = 5) WITH deployed airbag
# -----------------------------
st.subheader("Severe Prediction • With Deployed Airbag (Front)")
...
# -----------------------------
# PDP: Airbag — Severe (target = 5) WITHOUT deployed airbag
# -----------------------------
st.subheader("Severe Prediction • Without Deployed Airbag (Front)")
